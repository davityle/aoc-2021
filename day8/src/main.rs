...
fn main() {
    let mut numbers_by_lengths: HashMap<usize, &[usize]> = HashMap::new();

    numbers_by_lengths.insert(2, &[1]);
    numbers_by_lengths.insert(4, &[4]);
    numbers_by_lengths.insert(3, &[7]);
    numbers_by_lengths.insert(7, &[8]);
    numbers_by_lengths.insert(6, &[0, 6, 9]);
    numbers_by_lengths.insert(5, &[2, 3, 5]);
    ...
    for line in INPUT {
        ...
        let mut matches_by_slice: HashMap<&[usize], Vec<&str>> = HashMap::new();

        for word in parts[0].split(" ") {
            match numbers_by_lengths.get(&word.len()) {
                Some(val) => match matches_by_slice.entry(&val) {
                    Entry::Vacant(e) => {
                        e.insert(vec![word]);
                    }
                ...
                _ => println!("ya done messed up"),
            };
        }

        let one_match = matches_by_slice.get(get_slice(&[1])).unwrap()[0];
        let four_match = matches_by_slice.get(get_slice(&[4])).unwrap()[0];
        let seven_match = matches_by_slice.get(get_slice(&[7])).unwrap()[0];
        let eight_match = matches_by_slice.get(get_slice(&[8])).unwrap()[0];

        let right_chars = one_match.chars().collect::<Vec<_>>();

        let zero_six_nine_matches = matches_by_slice.get(get_slice(&[0, 6, 9])).unwrap();
        let zero_nine_matches = zero_six_nine_matches
            .iter()
            .map(|&word| word)
            .filter(|word| right_chars.iter().all(|&c| word.contains(c)))
            .collect::<Vec<_>>();
        let six_match = zero_six_nine_matches
            .iter()
            .find(|word| !zero_nine_matches.contains(word))
            .map(|&word| word)
            .unwrap();

        let two_three_five_matches = matches_by_slice.get(get_slice(&[2, 3, 5])).unwrap();

        let three_match = two_three_five_matches
            .iter()
            .map(|&word| word)
            .find(|word| right_chars.iter().all(|&c| word.contains(c)))
            .unwrap();

        let two_five_matches = two_three_five_matches
            .iter()
            .map(|&word| word)
            .filter(|&word| word != three_match)
            .collect::<Vec<_>>();

        let left_chars = eight_match
            .chars()
            .filter(|c| !three_match.chars().contains(c))
            .collect::<Vec<_>>();

        let zero_match = zero_nine_matches
            .iter()
            .map(|&word| word)
            .find(|word| left_chars.iter().all(|&c| word.contains(c)))
            .unwrap();

        let nine_match = zero_nine_matches
            .iter()
            .map(|&word| word)
            .find(|&word| word != zero_match)
            .unwrap();

        let top_right = right_chars
            .iter()
            .find(|c| !six_match.chars().contains(c))
            .unwrap();

        let two_match = two_five_matches
            .iter()
            .find(|word| word.chars().contains(top_right))
            .map(|&word| word)
            .unwrap();

        let five_match = two_five_matches
            .iter()
            .map(|&word| word)
            .find(|&word| word != two_match)
            .unwrap();

        let mut matches: HashMap<Vec<char>, &str> = HashMap::new();
        matches.insert(get_sorted_vec(zero_match), "0");
        matches.insert(get_sorted_vec(one_match), "1");
        matches.insert(get_sorted_vec(two_match), "2");
        matches.insert(get_sorted_vec(three_match), "3");
        matches.insert(get_sorted_vec(four_match), "4");
        matches.insert(get_sorted_vec(five_match), "5");
        matches.insert(get_sorted_vec(six_match), "6");
        matches.insert(get_sorted_vec(seven_match), "7");
        matches.insert(get_sorted_vec(eight_match), "8");
        matches.insert(get_sorted_vec(nine_match), "9");

        let mut output_value: String = "".to_owned();

        for input_word in parts[1].split(" ") {
            match matches.get(&get_sorted_vec(input_word)) {
                Some(val) => output_value.push_str(val),
                _ => println!("ya done messed up"),
            }
        }
    ...
    println!(
        ...
        output_values
            .iter()
            .flat_map(|val| val.parse::<u64>())
            .fold(0, |a, b| a + b)
    );
}

fn get_sorted_vec(slice: &str) -> Vec<char> {
    let mut chars: Vec<char> = slice.chars().collect();
    chars.sort_by(|a, b| b.cmp(a));
    chars
}

fn get_slice(a: &[usize]) -> &[usize] {
    a
}
